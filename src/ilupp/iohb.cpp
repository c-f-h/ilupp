...
/*           *strchr(Valfmt,'D') = 'E'; */
       ...
       for (ind = 0;ind<Valperline;ind++)
       {
	     ...
	     // ERROR IN ORIGINAL FILE: WAS for (j=last+1;j>=0;j--)
             for (j=last-1;j>=0;j--) {
...
// this has been modified, as memory will be freed with delete
...
try {
	int Nrhs;
        char *Type;
        int file_good;
	file_good = readHB_info(filename, M, N, nonzeros, &Type, &Nrhs);
        if(file_good == 0) return 0;
...
*/
    ...
    std::cerr<<"readHB_newmat_double: error allocating memory."<<std::endl;
    return 0;
}
