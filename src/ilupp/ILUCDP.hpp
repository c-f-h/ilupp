        ...
        Real mem_factor, Real& total_memory_allocated, Real& total_memory_used)
{
    ...
    for(k=0;k<n;k++){
        ...
        if(eliminate){  // select potential pivot
            val_larg_el=z.abs_max(pos_pivot); // finds largest element by absolute value. Returns value and position in z.
            if(non_pivot[selected_row]){  // not pivoting is with respect to the diagonal element of the selected row (if possible)
                if (std::abs(val_larg_el*piv_tol)>std::abs(z[selected_row]) && pos_pivot>=0 && IP.get_piv_tol() > 0){
                    //pivoting = true;
                    pivot = val_larg_el;
                } else {
                    //pivoting = false;
        ...
        /*
           ...
           if (std::abs(val_larg_el*piv_tol)>std::abs(z[perm[k]]) && pos_pivot>=0 && IP.get_piv_tol() > 0){
           pivoting = true;
           pivot = val_larg_el;
           } else {
           pivoting = false;
        ...
        Real mem_factor, Real& total_memory_allocated, Real& total_memory_used)
{
    ...
    std::cout<<"     pivot tolerance        = "<<piv_tol<<std::endl;
...
/*
template<class T>
bool matrix_sparse<T>::ILUCDPinv(const matrix_sparse<T>& Arow, const matrix_sparse<T>& Acol, matrix_sparse<T>& U, index_list& perm, index_list& permrows, Integer max_fill_in, Real threshold, Real piv_tol,  Integer bpr, Integer& zero_pivots, Real& time_self, Real mem_factor){
    try {
      clock_t time_begin, time_end;
      time_begin=clock();
      #ifdef VERBOSE
